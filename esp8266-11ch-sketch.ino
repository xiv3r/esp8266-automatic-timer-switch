 ...
 *  Storage: LittleFS with atomic writes & production reliability
 *  Feature: Day-of-Week Schedules (v5)
============================================================
 */
...
#define EEPROM_VERSION 5  // Incremented for day-of-week support
...
struct TimerSchedule {
    ...
    bool    enabled[8];
    uint8_t days[8];      // Bitmask: bit 0=Sun, 1=Mon, ... 6=Sat, 0x7F=All days
};
struct RelayConfig {
...
@media(max-width:500px){.grid{grid-template-columns:1fr}.ibar{grid-template-columns:1fr}.input-row{flex-direction:column}.day{width:24px;height:22px;font-size:10px}}
)css";

// ─────────────────────────────────────────────────────────────────────────────
//  INDEX PAGE (with day-of-week support)
// ─────────────────────────────────────────────────────────────────────────────
...
function nightBadge(sc){
  ...
  if(a===b)return'<span class="night always">&#x25CF; Always ON ('+ds+')</span>';
  if(a>b) return'<span class="night">&#x1F319; Overnight ('+ds+')</span>';
  return'<span class="night">&#x1F319; '+ds+'</span>';
}
...
// ─────────────────────────────────────────────────────────────────────────────
//  WIFI PAGE (Unchanged from original)
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
//  NTP / TIME PAGE (Unchanged from original)
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
//  AP PAGE (Unchanged from original)
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
//  SYSTEM PAGE (Unchanged from original)
// ─────────────────────────────────────────────────────────────────────────────
...
// =============================================================================
//  ATOMIC FILE I/O WITH POWER-LOSS PROTECTION (Unchanged)
// =============================================================================
...
bool loadFromFile(const char* path, void* data, size_t size) {
    ...
    size_t bytesRead = f.read((uint8_t*)data, size);
    f.close();
...
// =============================================================================
//  OPTIMIZED CONFIGURATION SAVES (Unchanged)
// =============================================================================
...
void flushConfigIfNeeded() {
    if (configDirty && (millis() - lastConfigSave >= CONFIG_SAVE_INTERVAL)) {
        ...
        saveToFileAtomic(RELAY_FILE, relayConfigs, sizeof(RelayConfig) * NUM_RELAYS);
        configDirty = false;
        yield();
    }
}
...
void setup() {
    ...
    for (int i = 0; i < NUM_RELAYS; i++) {
        for (int s = 0; s < 8; s++) {
            ...
            relayConfigs[i].schedule.days[s] = 0x7F;  // All days by default
        }
...
// =============================================================================
//  SCHEDULE ENGINE (MODIFIED for day-of-week support)
// =============================================================================
...
void processRelaySchedules() {
    ...
    int weekday = ti->tm_wday;  // 0=Sunday, 1=Monday, ... 6=Saturday
...
// =============================================================================
//  CONFIGURATION (MODIFIED for day-of-week migration)
// =============================================================================
...
void loadConfiguration() {
    ...
    // Migrate from v4 to v5 (add day-of-week support)
    if (sysConfig.version < 5) {
        sysConfig.version = EEPROM_VERSION;
        saveConfiguration();
    }
    
    if (!loadFromFile(RELAY_FILE, relayConfigs, sizeof(RelayConfig) * NUM_RELAYS)) {
        for (int i = 0; i < NUM_RELAYS; i++) {
            memset(&relayConfigs[i], 0, sizeof(RelayConfig));
            sprintf(relayConfigs[i].name, "Relay %d", i + 1);
            for (int s = 0; s < 8; s++) {
        ...
        // Ensure backward compatibility: if days field is 0, set it to all days
        for (int i = 0; i < NUM_RELAYS; i++) {
            if (strlen(relayConfigs[i].name) == 0) {
                sprintf(relayConfigs[i].name, "Relay %d", i + 1);
            }
            for (int s = 0; s < 8; s++) {
                if (relayConfigs[i].schedule.days[s] == 0) {
                    relayConfigs[i].schedule.days[s] = 0x7F;  // Backward compat: all days
                }
            }
        }
    }
...
// =============================================================================
//  JSON PARSING HELPERS (Added extractJsonByte)
// =============================================================================
...
// =============================================================================
//  API HANDLERS (MODIFIED for day-of-week support)
// =============================================================================

void handleGetRelays() {
    String resp;
    resp.reserve(3072);  // Increased for day data
    resp = "[";
    for (int i = 0; i < NUM_RELAYS; i++) {
        if (i > 0) resp += ",";
        resp += "{";
