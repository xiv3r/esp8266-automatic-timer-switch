...
#define EEPROM_VERSION 8  // Incremented for monthOfYear support
...
static const unsigned long HEAP_DEFRAG_INTERVAL = 7200000UL;     // 2 hours
static const unsigned long CONNECTION_CLEANUP_INTERVAL = 300000UL; // 5 minutes
static const unsigned long IDLE_CONNECTION_TIMEOUT = 60000UL;     // 1 minute
...
static const uint8_t       API_RATE_LIMIT = 10;           // Max requests/second
static const unsigned long RATE_LIMIT_WINDOW = 1000UL;    // 1 second window
...
struct TimerSchedule {
    ...
    uint16_t monthOfYear[8];  // NEW: 12-bit mask (bit 0=Jan, bit 11=Dec), 0xFFF = all months
};
struct RelayConfig {
...
.month{width:36px;height:22px;border-radius:3px;border:1px solid #CFD8DC;display:flex;align-items:center;justify-content:center;font-size:9px;font-weight:600;cursor:pointer;background:#FAFAFC;transition:.15s;user-select:none;text-transform:uppercase}
.month:hover{border-color:#EF5350;background:#FFEBEE}
.month.on{background:#C62828;color:#fff;border-color:#C62828}
...
@media(max-width:500px){.grid{grid-template-columns:1fr}.ibar{grid-template-columns:1fr}.input-row{flex-direction:column}.day{width:24px;height:22px;font-size:10px}.mday{width:22px;height:20px;font-size:9px}.month{width:30px;height:20px;font-size:8px}}
)css";
...
function monthOfYearMaskToStr(moy){
  if(moy===0xFFFF) return 'Every month';
  if(moy===0) return '';
  let s='';
  for(let i=0;i<12;i++) if(moy&(1<<i)) s+=M[i]+',';
  return s.replace(/,$/,'')||'None';
}
...
function nightBadge(sc){
  ...
  const my=monthOfYearMaskToStr(sc.monthOfYear||0xFFF);
  let info=ds;
  if(ms) info+=' | Days:'+ms;
  if(my) info+=' | '+my;
...
<div class="slist">`;
    for(let s=0;s<NS;s++){
      ...
      const monthOfYearBits = sc2.monthOfYear || 0xFFF;
...
<div class="months" id="month_${i}_${s}">`;
      for(let m=0;m<12;m++){
        const mask = 1<<m;
        html+=`<div class="month${(monthOfYearBits&mask)?' on':''}" onclick="toggleMonth(${i},${s},${m})" title="${M[m]}">${M[m]}</div>`;
      }
      html+=`</div>
</div>`;
    }
...
function toggleMonth(ri,si,monthIdx){
  ...
  if(!relays[ri].schedules[si].monthOfYear) relays[ri].schedules[si].monthOfYear = 0xFFF;
  relays[ri].schedules[si].monthOfYear ^= mask;
  const monthEl = document.getElementById('month_'+ri+'_'+si).children[monthIdx];
  if(monthEl) monthEl.className = 'month' + ((relays[ri].schedules[si].monthOfYear & mask)?' on':'');
  const nb=document.getElementById('nb_'+ri+'_'+si);
  if(nb)nb.innerHTML=nightBadge(relays[ri].schedules[si]);
}
...
// =============================================================================
//  NEW: Self-Recovery Functions (No Restarts)
// =============================================================================

void selfRecoveryWifiReconnect() {
    // Non-destructive WiFi reconnection
    if (wifiConnected) {
        // WiFi is actually connected, just refresh
        return;
    }
    
    // Try reconnection without disrupting AP
    WiFiMode_t currentMode = WiFi.getMode();
    
    if (strlen(sysConfig.sta_ssid) > 0) {
        // Ensure STA mode is enabled
...
void selfRecoveryWebServer() {
    ...
    // Non-destructive web server recovery
    // Just reinitialize the server without stopping WiFi
    server.stop();
    server.close();
    delay(50);
    setupWebServer();
...
void selfRecoveryMemoryCleanup() {
    // Aggressive memory cleanup without restart
    ESP.resetFreeContStack();
    
    // Clear any string fragmentation
    for (int i = 0; i < 5; i++) {
        String dummy;
        dummy.reserve(64);
        dummy = " ";
...
void selfRecoveryNetworkHeal() {
    // Gentle network healing without restart
...
void checkWebServerHealth() {
    ...
    // Simple health check - if heap critically low, recover web server
...
void checkDNSHealth() {
    ...
    // Check if DNS is still functioning
    // If we have very low memory, restart DNS
...
void defragmentHeap() {
    ...
    // Only defrag when needed
    if (ESP.getFreeHeap() > 20480) return;
    
    // Force memory cleanup
    ESP.resetFreeContStack();
    
    // Allocate and free to coalesce fragmented blocks
    for (int i = 0; i < 3; i++) {
        String dummy;
        dummy.reserve(128);
        dummy = " ";
...
void cleanupIdleConnections() {
    // The ESP8266WebServer manages connections internally,
    // but we can help by ensuring we're not keeping stale ones
    if (ESP.getFreeHeap() < 10240) {
        // Force close any lingering connections when memory is tight
        server.client().stop();
        delay(10);
    }
}
...
// =============================================================================
//  ALL EXISTING FUNCTIONS (UNCHANGED except no more restarts)
// =============================================================================
    ...
    for (int i = 0; i < MAX_RELAYS; i++) {
        ...
        if (i < 8) {
            ...
            for (int s = 0; s < 8; s++) {
                ...
                relayConfigs[i].schedule.monthOfYear[s] = 0xFFF;  // NEW: all months by default
    ...
    if (!loadFromFile(RELAY_FILE, relayConfigs, sizeof(RelayConfig) * MAX_RELAYS)) {
        ...
        for (int i = 0; i < numRelays && i < 8; i++) {
            ...
            for (int s = 0; s < 8; s++) {
                relayConfigs[i].schedule.days[s] = 0x7F;
                relayConfigs[i].schedule.monthDays[s] = 0;
                relayConfigs[i].schedule.monthOfYear[s] = 0xFFF;  // NEW
                relayConfigs[i].schedule.enabled[s] = false;
            }
        }
    } else {
        // Check if monthOfYear exists in saved config
        // If all values are 0, treat as old config and set default
        bool hasMonthOfYear = false;
        for (int i = 0; i < numRelays && i < MAX_RELAYS && !hasMonthOfYear; i++) {
            for (int s = 0; s < 8 && !hasMonthOfYear; s++) {
                if (relayConfigs[i].schedule.monthOfYear[s] != 0) {
                    hasMonthOfYear = true;
                }
            }
        }
        ...
        if (!hasMonthOfYear) {
            // Old config without monthOfYear - set all months enabled
            for (int i = 0; i < numRelays && i < MAX_RELAYS; i++) {
                for (int s = 0; s < 8; s++) {
                    relayConfigs[i].schedule.monthOfYear[s] = 0xFFF;
                }
            }
        }
    }
...
// =============================================================================
//  ENHANCED LOOP with self-recovery instead of restarts
// =============================================================================
...
void loop() {
    ...
    // NEW: Periodic memory and connection management
    defragmentHeap();
    manageConnections();
    ...
    // Self-recovery state machine (replaces restart-based recovery)
    if (selfRecoveryState != SR_IDLE) {
    ...
    // Auto-heal checks using self-recovery instead of liveReconfigure
    // WiFi health check
...
// =============================================================================
//  SCHEDULE PROCESSING (UPDATED with monthOfYear)
// =============================================================================
...
void processRelaySchedules() {
    ...
    int monthOfYear = ti->tm_mon + 1;  // tm_mon is 0-11, we want 1-12
    ...
    for (int i = 0; i < numRelays; i++) {
        ...
        for (int s = 0; s < 8; s++) {
            ...
            // NEW: Check month of year
            uint16_t monthMask = relayConfigs[i].schedule.monthOfYear[s];
            // If monthMask is 0, treat as all months (backward compatibility)
            if (monthMask == 0) monthMask = 0xFFF;
            if (!(monthMask & (1 << (monthOfYear - 1)))) continue;
...
// =============================================================================
//  WEB SERVER HANDLERS (ENHANCED with rate limiting and graceful degradation)
// =============================================================================
...
// =============================================================================
//  ENHANCED API HANDLERS with rate limiting and graceful degradation
// =============================================================================
...
void handleGetRelays() {
    ...
    for (int i = 0; i < numRelays; i++) {
        ...
        for (int s = 0; s < 8; s++) {
            ...
            resp += "\"monthOfYear\":" + String(relayConfigs[i].schedule.monthOfYear[s]);  // NEW
            resp += "}";
        }
        resp += "]}";
    }
...
void handleSaveRelay() {
    ...
    for (int s = 0; s < 8; s++) {
        ...
        relayConfigs[relay].schedule.monthOfYear[s] = extractJsonInt(schedStr, "monthOfYear");  // NEW
        pos = endPos + 1;
    }
    markConfigDirty();
    server.send(200, "application/json", "{\"success\":true}");
...
void handleGetSystem() {
    ...
    // Graceful degradation for system info
    if (shouldSimplifyResponse()) {
        char buf[128];
...
void handleReset() {
    ...
    // Use self-recovery WiFi reconnect instead of restart
    selfRecoveryState = SR_WIFI_RECONNECT;
}
...
// Soft reset now uses self-recovery
void handleSoftReset() {
    if (!checkApiRateLimit()) {
        server.send(429, "application/json", "{\"success\":false}");
        return;
    ...
    // Trigger full self-recovery sequence
