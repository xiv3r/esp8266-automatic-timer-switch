...
static const unsigned long MIN_HEAP_FOR_MQTT = 10240UL;
static const unsigned long MQTT_RECONNECT_INTERVAL = 5000UL;
static const unsigned long MQTT_STATUS_INTERVAL = 60000UL;
...
// =============================================================================
//  Non-Blocking State Machine Enumerations
// =============================================================================
...
// =============================================================================
//  NTP Server Fallback Configuration
// =============================================================================
...
bool connectMQTT();
void handleMQTT();
void publishRelayStateMQTT(int relay);
void publishAllRelayStatesMQTT();
void publishSystemStatusMQTT();
...
<div class="hdr-r"><span class="dot wd"></span><span class="dot nd"></span><span id="mqttDot" class="dot"></span>&nbsp;<span id="clk">--:--:--</span></div>
</header>
<main>
...
<div class="fg"><label>Broker Address</label><input type="text" id="broker" placeholder="192.168.1.100 or mqtt.local"></div>
<div class="fg"><label>Port</label><input type="number" id="port" value="1883" min="1" max="65535"></div>
<div class="fg"><label>Username (optional)</label><input type="text" id="username" placeholder="Leave blank if none"></div>
<div class="fg"><label>Password (optional)</label><input type="password" id="password" placeholder="Leave blank if none"></div>
<div class="fg"><label>Client ID (optional)</label><input type="text" id="clientId" placeholder="Auto-generated if blank"></div>
<div class="fg"><label>Base Topic</label><input type="text" id="baseTopic" value="esp8266/relays"></div>
<div class="fg"><label>QoS (0-2)</label>
<select id="qos"><option value="0">0 - At most once</option><option value="1" selected>1 - At least once</option><option value="2">2 - Exactly once</option></select></div>
<div class="fg"><label>Retain Messages</label>
<select id="retain"><option value="0">No</option><option value="1" selected>Yes</option></select></div>
<div class="fg"><label>Home Assistant Auto-Discovery</label>
<select id="haDiscovery"><option value="1" selected>Enabled</option><option value="0">Disabled</option></select></div>
<div style="display:flex;gap:8px;flex-wrap:wrap">
<button class="btn bsave" style="flex:1" onclick="save()">&#x1F4BE; Save Settings</button>
<button class="btn bsync" onclick="testMQTT()" style="padding:9px 16px;border-radius:6px;font-size:13px;font-weight:600">&#x1F504; Test Connection</button>
</div>
</div>
<div class="card fcrd" style="margin-top:14px">
<h3 style="margin-bottom:8px;color:#1565C0">MQTT Topics</h3>
<div style="font-family:monospace;font-size:12px;line-height:1.8">
<p><strong>Command Topics:</strong></p>
<p style="color:#1565C0">{baseTopic}/0/set &rarr; ON/OFF</p>
<p style="color:#1565C0">{baseTopic}/0/manual &rarr; true/false</p>
<p style="color:#1565C0">{baseTopic}/0/reset &rarr; (auto mode)</p>
<p style="color:#1565C0">{baseTopic}/all/set &rarr; ON/OFF (all relays)</p>
<p style="margin-top:8px"><strong>Status Topics:</strong></p>
<p style="color:#2E7D32">{baseTopic}/0/state &rarr; ON/OFF</p>
<p style="color:#2E7D32">{baseTopic}/0/name &rarr; Relay name</p>
<p style="color:#2E7D32">{baseTopic}/status &rarr; online/offline</p>
<p style="color:#2E7D32">{baseTopic}/system &rarr; JSON status</p>
</div>
</div>
</main>
<div id="toast"></div>
...
function loadMQTTStatus(){
  fetch('/api/mqtt').then(r=>r.json()).then(d=>{
    document.getElementById('enabled').value=d.enabled?'1':'0';
    ...
    document.getElementById('baseTopic').value=d.baseTopic||'esp8266/relays';
    document.getElementById('qos').value=d.qos||1;
    document.getElementById('retain').value=d.retain?'1':'0';
    document.getElementById('haDiscovery').value=d.haDiscovery?'1':'0';
    const dot=document.getElementById('mqttDot');
    if(dot)dot.className='dot '+(d.connected?'g':'r');
  }).catch(()=>{});
}
loadMQTTStatus();
...
function save(){
  fetch('/api/mqtt',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({
    ...
    retain:document.getElementById('retain').value==='1',
    haDiscovery:document.getElementById('haDiscovery').value==='1'
  })}).then(r=>r.json()).then(d=>{
    if(d.success)toast('MQTT settings saved!');
...
function testMQTT(){
  ...
  fetch('/api/mqtt/test',{method:'POST'}).then(r=>r.json()).then(d=>{
    btn.disabled=false;
    btn.innerHTML='&#x1F504; Test Connection';
    if(d.success)toast('MQTT connection successful!');
    else toast('MQTT test failed: '+(d.error||'unknown'),false);
  }).catch(()=>{
    btn.disabled=false;
    btn.innerHTML='&#x1F504; Test Connection';
    toast('Error testing MQTT',false);
  });
}
</script></body></html>)raw";
...
function loadSys(){
  fetch('/api/system').then(r=>r.json()).then(d=>{
    ...
    document.getElementById('smq').textContent=d.mqttConnected?'Connected':'Disconnected';
...
void processFactoryReset() {
    ...
    switch (frState) {
        ...
        case FR_DELETE_FILES:
            ...
            if (LittleFS.exists(MQTT_CONFIG_FILE)) LittleFS.remove(MQTT_CONFIG_FILE);
...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
    ...
    String baseTopic = mqttConfig.baseTopic;
    
    int relayNum = -1;
    if (topicStr.startsWith(baseTopic + "/")) {
        String subPath = topicStr.substring(baseTopic.length() + 1);
        int slashPos = subPath.indexOf('/');
        if (slashPos > 0) {
            String relayStr = subPath.substring(0, slashPos);
            if (relayStr == "all") {
                relayNum = -2;
            } else {
            ...
            String action = subPath.substring(slashPos + 1);
            
            if (action == "set" || action == "command") {
                bool state = (payloadStr == "ON" || payloadStr == "1" || 
                             payloadStr == "true" || payloadStr == "on");
                bool manual = (action == "set");
                
                if (relayNum == -2) {
                    for (int i = 0; i < numRelays; i++) {
                        relayConfigs[i].manualOverride = manual;
                        relayConfigs[i].manualState = state;
                    }
                } else if (relayNum >= 0 && relayNum < numRelays) {
                    relayConfigs[relayNum].manualOverride = manual;
                    relayConfigs[relayNum].manualState = state;
                }
                markConfigDirty();
                publishAllRelayStatesMQTT();
            }
                ...
                if (relayNum == -2) {
                    ...
                    relayConfigs[relayNum].manualOverride = false;
                }
                markConfigDirty();
                publishAllRelayStatesMQTT();
            }
...
bool connectMQTT() {
    if (!mqttConfig.enabled || !wifiConnected) return false;
    if (strlen(mqttConfig.broker) == 0) return false;
    if (ESP.getFreeHeap() < MIN_HEAP_FOR_MQTT) return false;
    
    mqttClient.setServer(mqttConfig.broker, mqttConfig.port);
    mqttClient.setCallback(mqttCallback);
    mqttClient.setBufferSize(512);
    
    String clientId = String(mqttConfig.clientId);
    if (clientId.isEmpty()) {
        clientId = "ESP8266-" + String(ESP.getChipId(), HEX);
    }
    
    String lwtTopic = String(mqttConfig.baseTopic) + "/status";
    
    bool connected = false;
    if (strlen(mqttConfig.username) > 0) {
        connected = mqttClient.connect(clientId.c_str(), 
            mqttConfig.username, mqttConfig.password,
            lwtTopic.c_str(), mqttConfig.qos, true, "offline");
    } else {
        connected = mqttClient.connect(clientId.c_str(),
            lwtTopic.c_str(), mqttConfig.qos, true, "offline");
    }
    ...
    if (connected) {
        String baseTopic = mqttConfig.baseTopic;
        mqttClient.subscribe((baseTopic + "/+/set").c_str(), mqttConfig.qos);
        mqttClient.subscribe((baseTopic + "/+/command").c_str(), mqttConfig.qos);
        mqttClient.subscribe((baseTopic + "/+/reset").c_str(), mqttConfig.qos);
        mqttClient.subscribe((baseTopic + "/all/set").c_str(), mqttConfig.qos);
        
        mqttClient.publish(lwtTopic.c_str(), "online", true);
        mqttConnected = true;
        
        publishAllRelayStatesMQTT();
        publishSystemStatusMQTT();
...
void handleMQTT() {
    if (!mqttConfig.enabled || !wifiConnected) return;
    if (ESP.getFreeHeap() < MIN_HEAP_FOR_MQTT) {
        if (mqttClient.connected()) {
            mqttClient.disconnect();
            mqttConnected = false;
        }
    ...
    if (now - lastMQTTStatusPublish >= MQTT_STATUS_INTERVAL) {
...
void publishRelayStateMQTT(int relay) {
    if (!mqttConnected || relay < 0 || relay >= numRelays) return;
    
    String baseTopic = mqttConfig.baseTopic;
    bool relayState = digitalRead(relayConfigs[relay].pin);
    if (relayConfigs[relay].activeLow) relayState = !relayState;
    
    String statePayload = relayState ? "ON" : "OFF";
    
    mqttClient.publish((baseTopic + "/" + String(relay) + "/state").c_str(), 
        statePayload.c_str(), mqttConfig.retain);
    mqttClient.publish((baseTopic + "/" + String(relay) + "/name").c_str(), 
        relayConfigs[relay].name, mqttConfig.retain);
}

void publishAllRelayStatesMQTT() {
    if (!mqttConnected) return;
    for (int i = 0; i < numRelays; i++) {
        publishRelayStateMQTT(i);
    }
}
...
void publishSystemStatusMQTT() {
    ...
    mqttClient.publish((baseTopic + "/system").c_str(), 
        payload.c_str(), mqttConfig.retain);
}

void publishHADiscovery() {
    if (!mqttConnected || !mqttConfig.haDiscovery) return;
    
    String baseTopic = mqttConfig.baseTopic;
    String deviceId = String(ESP.getChipId(), HEX);
    
    for (int i = 0; i < numRelays; i++) {
        String relayTopic = baseTopic + "/" + String(i);
        
        DynamicJsonDocument doc(1024);
        doc["name"] = relayConfigs[i].name;
        doc["unique_id"] = deviceId + "_relay_" + String(i);
        doc["command_topic"] = relayTopic + "/set";
        doc["state_topic"] = relayTopic + "/state";
        doc["payload_on"] = "ON";
        doc["payload_off"] = "OFF";
        doc["state_on"] = "ON";
        doc["state_off"] = "OFF";
        doc["qos"] = mqttConfig.qos;
        doc["retain"] = mqttConfig.retain;
        
        JsonObject device = doc.createNestedObject("device");
        device["identifiers"] = deviceId;
        device["name"] = sysConfig.hostname;
        device["model"] = "ESP8266 8CH Relay";
        device["manufacturer"] = "ESP8266";
        device["sw_version"] = String(EEPROM_VERSION);
        
        String discoPayload;
        serializeJson(doc, discoPayload);
        
        String discoTopic = "homeassistant/switch/" + deviceId + 
            "/relay_" + String(i) + "/config";
        
        mqttClient.publish(discoTopic.c_str(), discoPayload.c_str(), true);
    }
}

void loadMQTTConfig() {
    if (!loadFromFile(MQTT_CONFIG_FILE, &mqttConfig, sizeof(MQTTConfig))) {
        memset(&mqttConfig, 0, sizeof(MQTTConfig));
        mqttConfig.magic = MQTT_MAGIC;
        mqttConfig.enabled = false;
        mqttConfig.port = 1883;
        strcpy(mqttConfig.baseTopic, "esp8266/relays");
        mqttConfig.qos = 1;
        mqttConfig.retain = true;
        mqttConfig.haDiscovery = true;
        saveMQTTConfig();
        return;
    }
    ...
    if (mqttConfig.qos > 2) mqttConfig.qos = 1;
}

void saveMQTTConfig() {
    saveToFileAtomic(MQTT_CONFIG_FILE, &mqttConfig, sizeof(MQTTConfig));
}
...
void loop() {
    ...
    // MQTT Handling - runs at lowest priority
...
void setupWebServer() {
    ...
    server.on("/api/mqtt/test", HTTP_POST, handleTestMQTT);
...
void handleSaveRelayName() {
    ...
    if (relay >= 0 && relay < numRelays && name && strlen(name) > 0) {
        ...
        markConfigDirty();
        if (mqttConnected) publishRelayStateMQTT(relay);
...
void handleGetMQTT() {
    ...
    server.send(200, "application/json", 
        "{\"enabled\":" + String(mqttConfig.enabled ? "true" : "false") +
        ",\"broker\":\"" + String(mqttConfig.broker) + "\"" +
        ",\"port\":" + String(mqttConfig.port) +
        ",\"username\":\"" + String(mqttConfig.username) + "\"" +
        ",\"password\":\"" + String(mqttConfig.password) + "\"" +
        ",\"clientId\":\"" + String(mqttConfig.clientId) + "\"" +
        ",\"baseTopic\":\"" + String(mqttConfig.baseTopic) + "\"" +
        ",\"qos\":" + String(mqttConfig.qos) +
        ",\"retain\":" + String(mqttConfig.retain ? "true" : "false") +
        ",\"haDiscovery\":" + String(mqttConfig.haDiscovery ? "true" : "false") +
        ",\"connected\":" + String(mqttConnected ? "true" : "false") + "}");
}
void handleSaveMQTT() {
    if (!checkApiRateLimit()) {
        server.send(429, "application/json", "{\"success\":false}");
    ...
    if (doc.containsKey("broker")) { strncpy(mqttConfig.broker, doc["broker"], 63); mqttConfig.broker[63] = '\0'; }
    if (doc.containsKey("port")) mqttConfig.port = doc["port"];
    if (doc.containsKey("username")) { strncpy(mqttConfig.username, doc["username"], 31); mqttConfig.username[31] = '\0'; }
    if (doc.containsKey("password")) { strncpy(mqttConfig.password, doc["password"], 31); mqttConfig.password[31] = '\0'; }
    if (doc.containsKey("clientId")) { strncpy(mqttConfig.clientId, doc["clientId"], 31); mqttConfig.clientId[31] = '\0'; }
    if (doc.containsKey("baseTopic")) { strncpy(mqttConfig.baseTopic, doc["baseTopic"], 63); mqttConfig.baseTopic[63] = '\0'; }
    if (doc.containsKey("qos")) mqttConfig.qos = doc["qos"];
    if (doc.containsKey("retain")) mqttConfig.retain = doc["retain"];
    if (doc.containsKey("haDiscovery")) mqttConfig.haDiscovery = doc["haDiscovery"];
    ...
    if (mqttConfig.enabled && wifiConnected) {
        connectMQTT();
    }
...
void handleTestMQTT() {
    ...
    if (!mqttConfig.enabled || !wifiConnected) {
        server.send(400, "application/json", "{\"success\":false,\"error\":\"MQTT not enabled or WiFi not connected\"}");
        return;
    }
