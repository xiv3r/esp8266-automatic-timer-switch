/*
===============================================================================
 *  ESP8266 6-Channel Relay Smart Switch w/ LED Status Indicator & MQTT & DS3231
 *  Author: Raff Alds
...
static const unsigned long MQTT_STATUS_PUBLISH_INTERVAL = 60000UL;
static const unsigned long MQTT_KEEPALIVE_SEC = 60;
...
static const unsigned long EMERGENCY_HEAP_THRESHOLD = 5120UL;
static const uint8_t       MAX_MQTT_RECONNECT_ATTEMPTS = 10;

// =============================================================================
//  MQTT Configuration
// =============================================================================
#define MQTT_ROOT_TOPIC           "esp8266relay"
#define MQTT_SUB_TOPIC_RELAY      MQTT_ROOT_TOPIC "/relay/#"
#define MQTT_PUB_TOPIC_STATUS     MQTT_ROOT_TOPIC "/status"
#define MQTT_PUB_TOPIC_RELAY      MQTT_ROOT_TOPIC "/relay"
#define MQTT_PUB_TOPIC_AVAIL      MQTT_ROOT_TOPIC "/available"
#define MQTT_PUB_TOPIC_TIME       MQTT_ROOT_TOPIC "/time"
#define MQTT_PUB_TOPIC_WIFI       MQTT_ROOT_TOPIC "/wifi"
#define MQTT_PUB_TOPIC_SYSTEM     MQTT_ROOT_TOPIC "/system"

// =============================================================================
//  WiFi Station Control
// =============================================================================
static const unsigned long WIFI_STATION_RECHECK_INTERVAL = 300000UL;
...
static const uint8_t MAX_SELF_RECOVERY_ATTEMPTS = 10;

// =============================================================================
//  Watchdog
// =============================================================================
static const unsigned long WATCHDOG_FEED_INTERVAL = 100UL;
...
unsigned long lastMqttTimePublish = 0;
...
unsigned long wifiStationDisabledTime = 0;
...
time_t lastNTPSyncEpochCal = 0;
unsigned long lastNTPSyncMsCal = 0;
...
uint64_t getMonotonicMs() {
    return ((uint64_t)millisOverflowCount << 32) | (uint64_t)millis();
}
...
inline bool checkFlashWriteRate() {
    unsigned long now = millis();
    if (now - flashWriteHourStart >= 3600000UL) {
        flashWriteCounter = 0;
...
void selfRecoveryDeepHeal();
void selfRecoveryFlashMaintenance();
...
void mqttPublishRelayState(int relay);
void mqttPublishAllRelays();
void mqttPublishTime();
void mqttPublishDiscovery();
int extractRelayFromTopic(const String& topic);
void handleMqttRelayCommand(int relay, const String& payload);
void handleMqttGlobalCommand(const String& payload);
String mqttPayloadToString(uint8_t* payload, unsigned int length);
...
.btoggle{background:#00897B;color:#fff;width:100%;padding:9px;font-size:13px;border-radius:6px;margin-top:8px}
...
<div class="hdr-r"><span class="dot wd"></span><span class="dot nd"></span><span class="dot md"></span>&nbsp;<span id="clk">--:--:--</span></div>
</header>
<main>
...
function tick(){fetch('/api/time').then(r=>r.json()).then(d=>{document.getElementById('clk').textContent=d.time||'--:--:--';const w=document.querySelector('.wd'),n=document.querySelector('.nd'),m=document.querySelector('.md');if(w)w.className='dot '+(d.wifi?'g':'r');if(n)n.className='dot '+(d.ntp?'g':'y');if(m)m.className='dot '+(d.mqtt?'b':'');}).catch(()=>{});}
...
<div class="card-hdr">
<span class="ctitle">WiFi Station Control</span>
<span class="badge" id="staBadge">ON</span>
</div>
<p style="font-size:12px;color:#90A4AE;margin-bottom:12px">
Turn off to use Access Point mode only. Relay schedules and AP remain active.
</p>
<button class="btn btoggle" id="toggleBtn" onclick="toggleStation()">Disable WiFi Station</button>
</div>
</main>
<div id="toast"></div>
<script>
...
function toggleStation(){
  ...
  .then(r=>r.json()).then(d=>{
    btn.disabled=false;
    if(d.success){
      const badge=document.getElementById('staBadge');
      const enabled=d.enabled;
      badge.textContent=enabled?'ON':'OFF';
      badge.className='badge '+(enabled?'bon':'boff');
      btn.textContent=enabled?'Disable WiFi Station':'Enable WiFi Station';
      toast(enabled?'WiFi station enabled':'WiFi station disabled');
      setTimeout(loadWiFiInfo,2000);
    }else toast('Toggle failed',false);
  }).catch(()=>{btn.disabled=false;btn.textContent='Toggle WiFi Station';toast('Error',false);});
}
...
function loadWiFiInfo(){
  fetch('/api/wifi').then(r=>r.json()).then(d=>{
    ...
    const btn=document.getElementById('toggleBtn');
    const badge=document.getElementById('staBadge');
    if(d.stationEnabled!==undefined){
      badge.textContent=d.stationEnabled?'ON':'OFF';
      badge.className='badge '+(d.stationEnabled?'bon':'boff');
      btn.textContent=d.stationEnabled?'Disable WiFi Station':'Enable WiFi Station';
    }
    ...
    if(d.connected){
      ...
      } else if(d.ssid && !d.stationEnabled) {
        s.textContent='WiFi station is disabled';
      } else if(d.ssid) {
...
<select id="mqttEnabled" style="flex:1">
<option value="1">Enabled</option>
<option value="0">Disabled</option>
</select>
<button class="btn btoggle" onclick="toggleMQTT()">Toggle MQTT</button>
</div>
</div>
<div class="fg"><label>Broker Address</label><input type="text" id="broker" placeholder="192.168.1.100"></div>
<div class="fg"><label>Port</label><input type="number" id="port" value="1883" min="1" max="65535"></div>
<div class="fg"><label>Username (optional)</label><input type="text" id="username" placeholder="mqtt user"></div>
<div class="fg"><label>Password (optional)</label><input type="password" id="password" placeholder="mqtt password"></div>
<div class="fg"><label>Client ID</label><input type="text" id="clientId" readonly></div>
<button class="btn bsave" onclick="saveMQTT()">&#x1F4BE; Save MQTT Settings</button>
</div>

<div class="card fcrd" style="margin-top:14px">
<p style="font-weight:700;margin-bottom:12px">MQTT Topics</p>
<div class="fg">
<small>Control Topic: <code>esp8266relay/relay/0/set</code> (payload: ON, OFF, TOGGLE, AUTO)</small>
<small>Command Topic: <code>esp8266relay/relay/0/command</code></small>
<small>Global Command: <code>esp8266relay/command</code> (ALL_ON, ALL_OFF, ALL_AUTO, STATUS)</small>
<small>Status Topic: <code>esp8266relay/status</code></small>
<small>State Topic: <code>esp8266relay/relay/0/state</code></small>
<small>Availability: <code>esp8266relay/available</code></small>
</div>
</div>
</main>
<div id="toast"></div>
...
function loadMQTT(){
  fetch('/api/mqtt').then(r=>r.json()).then(d=>{
    document.getElementById('mqttEnabled').value=d.enabled?'1':'0';
    ...
    s.style.display='';
    if(d.connected){
      s.innerHTML='&#x2705; MQTT Connected to <strong>'+d.broker+'</strong>';
      s.className='alert as';
    }else if(d.enabled){
      s.innerHTML='&#x26A0;&#xFE0F; MQTT Disconnected from <strong>'+d.broker+'</strong>';
      s.className='alert aw';
    }else{
      s.innerHTML='MQTT is disabled';
      s.className='alert ai';
    }
  }).catch(()=>{});
}
...
function toggleMQTT(){
  fetch('/api/mqtt/toggle',{method:'POST'}).then(r=>r.json()).then(d=>{
    if(d.success){
      toast('MQTT '+(d.enabled?'enabled':'disabled'));
      loadMQTT();
    }else toast('Toggle failed',false);
  }).catch(()=>toast('Error',false));
}
...
function saveMQTT(){
  fetch('/api/mqtt',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({
...
<div class="ibox"><div class="l">WiFi Station</div><div class="v" id="sws">&hellip;</div></div>
<div class="ibox"><div class="l">MQTT Status</div><div class="v" id="smq">&hellip;</div></div>
</div>
...
function loadSys(){
  fetch('/api/system').then(r=>r.json()).then(d=>{
    ...
    document.getElementById('sws').textContent=d.wifiStationEnabled?'Enabled':'Disabled';
    document.getElementById('smq').textContent=d.mqttConnected?'Connected':(d.mqttEnabled?'Disconnected':'Disabled');
...
void disableWiFiStation() {
    ...
    ntpFailCount = 0;
    wifiStationDisabledTime = millis();    
...
void selfRecoveryWebServer() {
    unsigned long now = millis();
    if (now - lastWebServerRecovery < WEB_SERVER_RECOVERY_INTERVAL) {
        selfRecoveryState = SR_IDLE;
        return;
    }
...
void selfRecoveryRTC() {
    if (ds3231Present) {
        syncFromDS3231();
    } else if (wifiConnected && wifiStationEnabled && ESP.getFreeHeap() > MIN_HEAP_FOR_NTP) {
        tryNTPSync();
    } else {
        if (internalEpoch > 0) {
...
void processFactoryReset() {
    ...
    switch (frState) {
        ...
        case FR_DELETE_FILES:
            ...
            if (LittleFS.exists(STATE_BACKUP)) LittleFS.remove(STATE_BACKUP);
            if (LittleFS.exists(MQTT_FILE)) LittleFS.remove(MQTT_FILE);
...
void restoreRelayState() {
    ...
    if (LittleFS.exists(STATE_BACKUP)) {
        ...
        if (f) {
            if (f.read((uint8_t*)&backup, sizeof(RelayStateBackup)) == sizeof(RelayStateBackup)) {
                if (backup.magic == STATE_MAGIC) {
                    for (int i = 0; i < numRelays; i++) {
...
void syncInternalRTC() {
    ...
    lastNTPSyncEpochCal = ntpEpoch;
    lastNTPSyncMsCal = nowMs;
    ntpFailCount = 0;
    saveRTCState();
...
void saveRTCState() {
    ...
    sysConfig.crystal_ppm = crystalDriftPPM;
    sysConfig.crystal_calibrated = crystalCalibrated;
    sysConfig.wifi_station_enabled = wifiStationEnabled;
    markConfigDirty();
}
void loadRTCState() {
...
void initDefaults() {
    ...
    memset(&mqttConfig, 0, sizeof(MqttConfig));
    mqttConfig.magic = MQTT_MAGIC;
    mqttConfig.port = 1883;
    mqttConfig.enabled = false;
    sprintf(mqttConfig.clientId, "ESP8266Relay_%06X", ESP.getChipId());
...
void loadPinConfig() {
    loadFromFile(PINS_FILE, &pinConfig, sizeof(PinConfig));
...
void loadMqttConfig() {
    loadFromFile(MQTT_FILE, &mqttConfig, sizeof(MqttConfig));
    if (mqttConfig.magic != MQTT_MAGIC) {
        memset(&mqttConfig, 0, sizeof(MqttConfig));
        mqttConfig.magic = MQTT_MAGIC;
        mqttConfig.port = 1883;
        mqttConfig.enabled = false;
...
void saveMqttConfig() {
    saveToFileAtomic(MQTT_FILE, &mqttConfig, sizeof(MqttConfig));
}
...
// =============================================================================
void setupMQTT() {
    if (!mqttEnabled || strlen(mqttConfig.broker) == 0) return;
...
void mqttLoop() {
    ...
    unsigned long now = millis();
    if (mqttClient.connected() && (now - lastMqttStatusPublish >= MQTT_STATUS_PUBLISH_INTERVAL)) {
        mqttPublishStatus();
        mqttPublishTime();
...
void mqttReconnect() {
    if (!wifiConnected || mqttReconnectAttempts >= MAX_MQTT_RECONNECT_ATTEMPTS) {
        return;
    }
    ...
    if (strlen(mqttConfig.username) > 0) {
        connected = mqttClient.connect(
            ...
            mqttConfig.username,
            mqttConfig.password,
            MQTT_PUB_TOPIC_AVAIL, 0, true, "offline"
        );
    } else {
        connected = mqttClient.connect(
            mqttConfig.clientId,
    ...
    if (connected) {
        ...
        mqttClient.subscribe((String(MQTT_SUB_TOPIC_RELAY) + "/+/set").c_str());
        mqttClient.subscribe((String(MQTT_SUB_TOPIC_RELAY) + "/+/command").c_str());
        mqttClient.subscribe((String(MQTT_ROOT_TOPIC) + "/command").c_str());
        
        mqttClient.publish(MQTT_PUB_TOPIC_AVAIL, "online", true);
        
        mqttPublishAllRelays();
        mqttPublishStatus();
        mqttPublishTime();
...
void mqttCallback(char* topic, uint8_t* payload, unsigned int length) {
    ...
    if (topicStr.indexOf((String(MQTT_PUB_TOPIC_RELAY) + "/")) != -1 && topicStr.indexOf("/set") != -1) {
        int relay = extractRelayFromTopic(topicStr);
        if (relay >= 0 && relay < numRelays) {
            handleMqttRelayCommand(relay, payloadStr);
        }
    }
    else if (topicStr.indexOf((String(MQTT_PUB_TOPIC_RELAY) + "/")) != -1 && topicStr.indexOf("/command") != -1) {
        int relay = extractRelayFromTopic(topicStr);
        if (relay >= 0 && relay < numRelays) {
            handleMqttRelayCommand(relay, payloadStr);
        }
    }
    else if (topicStr == (String(MQTT_ROOT_TOPIC) + "/command")) {
        handleMqttGlobalCommand(payloadStr);
    }
}

int extractRelayFromTopic(const String& topic) {
    int firstSlash = topic.indexOf('/', topic.indexOf('/') + 1);
    int secondSlash = topic.indexOf('/', firstSlash + 1);
    if (firstSlash != -1 && secondSlash != -1) {
        String relayStr = topic.substring(firstSlash + 1, secondSlash);
...
void handleMqttRelayCommand(int relay, const String& payload) {
    if (relay < 0 || relay >= numRelays) return;
    ...
    if (cmd == "ON" || cmd == "1" || cmd == "TRUE") {
        ...
        relayConfigs[relay].manualState = true;
        markConfigDirty();
        mqttPublishRelayState(relay);
    }
    else if (cmd == "OFF" || cmd == "0" || cmd == "FALSE") {
        relayConfigs[relay].manualOverride = true;
        relayConfigs[relay].manualState = false;
        markConfigDirty();
        mqttPublishRelayState(relay);
    }
    else if (cmd == "AUTO" || cmd == "RESET") {
        relayConfigs[relay].manualOverride = false;
        markConfigDirty();
        mqttPublishRelayState(relay);
    }
    else if (cmd == "TOGGLE") {
        bool currentState = digitalRead(relayConfigs[relay].pin);
        if (relayConfigs[relay].activeLow) currentState = !currentState;
        relayConfigs[relay].manualOverride = true;
        relayConfigs[relay].manualState = !currentState;
        markConfigDirty();
        mqttPublishRelayState(relay);
    }
        ...
        if (!deserializeJson(doc, payload)) {
            if (doc.containsKey("state")) {
                ...
                if (strcasecmp(state, "ON") == 0 || strcasecmp(state, "1") == 0) {
                    relayConfigs[relay].manualOverride = true;
                    relayConfigs[relay].manualState = true;
                } else if (strcasecmp(state, "OFF") == 0 || strcasecmp(state, "0") == 0) {
                    relayConfigs[relay].manualOverride = true;
                    relayConfigs[relay].manualState = false;
                } else if (strcasecmp(state, "AUTO") == 0) {
                    relayConfigs[relay].manualOverride = false;
                }
                markConfigDirty();
                mqttPublishRelayState(relay);
...
void mqttPublishRelayState(int relay) {
    if (!mqttClient.connected() || relay < 0 || relay >= numRelays) return;
    
    String topic = String(MQTT_PUB_TOPIC_RELAY) + "/" + String(relay);
    bool relayState = digitalRead(relayConfigs[relay].pin);
    if (relayConfigs[relay].activeLow) relayState = !relayState;
    
    StaticJsonDocument<256> doc;
    doc["relay"] = relay;
    doc["name"] = relayConfigs[relay].name;
    doc["state"] = relayState ? "ON" : "OFF";
    doc["manual"] = relayConfigs[relay].manualOverride;
    doc["pin"] = relayConfigs[relay].pin;
    
    String payload;
    serializeJson(doc, payload);
    mqttClient.publish(topic.c_str(), payload.c_str(), true);
    
    mqttClient.publish((topic + "/state").c_str(), relayState ? "ON" : "OFF", true);
}
...
void mqttPublishStatus() {
    ...
    doc["ds3231"] = ds3231Present;
    
    String payload;
    serializeJson(doc, payload);
    mqttClient.publish(MQTT_PUB_TOPIC_STATUS, payload.c_str(), true);
}
...
void mqttPublishTime() {
    ...
    doc["gmt_offset"] = sysConfig.gmt_offset;
    
    String payload;
    serializeJson(doc, payload);
    mqttClient.publish(MQTT_PUB_TOPIC_TIME, payload.c_str());
}
...
void mqttPublishDiscovery() {
    ...
    for (int i = 0; i < numRelays; i++) {
        String uniqueId = String("esp8266relay_") + String(ESP.getChipId(), HEX) + "_relay" + String(i);
        String discoveryTopic = "homeassistant/switch/" + uniqueId + "/config";
        
        StaticJsonDocument<512> doc;
        doc["name"] = String(relayConfigs[i].name);
        doc["unique_id"] = uniqueId;
        doc["command_topic"] = String(MQTT_PUB_TOPIC_RELAY) + "/" + String(i) + "/set";
        doc["state_topic"] = String(MQTT_PUB_TOPIC_RELAY) + "/" + String(i) + "/state";
        doc["availability_topic"] = MQTT_PUB_TOPIC_AVAIL;
        doc["payload_on"] = "ON";
        doc["payload_off"] = "OFF";
        doc["state_on"] = "ON";
        doc["state_off"] = "OFF";
        doc["json_attributes_topic"] = String(MQTT_PUB_TOPIC_RELAY) + "/" + String(i);
...
void loop() {
    ...
    if (selfRecoveryState != SR_IDLE) {
        switch (selfRecoveryState) {
            ...
            case SR_NETWORK_HEAL:
                ...
                break;
            case SR_DEEP_HEAL:
                selfRecoveryDeepHeal();
                break;
            case SR_FLASH_MAINTENANCE:
                selfRecoveryFlashMaintenance();
                break;
            default:
                selfRecoveryState = SR_IDLE;
                break;
    ...
    if (wifiStationEnabled && now - lastWiFiCheck >= WIFI_CHECK_INTERVAL) {
        lastWiFiCheck = now;
...
void setupWebServer() {
    ...
    server.on("/api/wifi/status", HTTP_GET,  handleWiFiStationStatus);
    ...
    server.on("/api/mqtt/toggle", HTTP_POST, handleMQTTToggle);  
...
void handleGetTime() {
    ...
    server.send(200, "application/json", "{\"time\":\"" + ts + "\",\"wifi\":" + String(wifiConnected ? "true" : "false") + ",\"ntp\":" + String(ntpSynced ? "true" : "false") + ",\"mqtt\":" + String(mqttConnected ? "true" : "false") + "}");
}
void handleBrowserTimeSync() {
    if (!checkApiRateLimit()) {
        server.send(429, "application/json", "{\"success\":false}");
...
void handleGetSystem() {
    ...
    if (shouldSimplifyResponse()) {
        ...
        snprintf(buf, sizeof(buf), 
            "{\"freeHeap\":%d,\"uptime\":%lu,\"wifiConnected\":%s,\"ds3231Present\":%s,\"wifiStationEnabled\":%s,\"mqttConnected\":%s,\"mqttEnabled\":%s}",
    ...
    resp += "\"mqttConnected\":" + String(mqttConnected ? "true" : "false") + ",";
    resp += "\"mqttEnabled\":" + String(mqttEnabled ? "true" : "false") + ",";
    if (ds3231Present) {
...
void handleGetMQTT() {
    ...
    doc["clientId"] = mqttConfig.clientId;
...
void handleSaveMQTT() {
    ...
    if (doc.containsKey("broker")) {
        strncpy(mqttConfig.broker, doc["broker"] | "", 63);
    ...
    if (doc.containsKey("username")) {
        strncpy(mqttConfig.username, doc["username"] | "", 31);
        mqttConfig.username[31] = '\0';
    }
    if (doc.containsKey("password")) {
        strncpy(mqttConfig.password, doc["password"] | "", 63);
        mqttConfig.password[63] = '\0';
    }
    if (doc.containsKey("enabled")) {
        mqttConfig.enabled = doc["enabled"];
        mqttEnabled = mqttConfig.enabled;
    }
...
void handleMQTTToggle() {
    ...
    mqttConfig.enabled = mqttEnabled;
    saveMqttConfig();
    
    if (!mqttEnabled && mqttClient.connected()) {
        mqttClient.publish(MQTT_PUB_TOPIC_AVAIL, "offline", true);
        mqttClient.disconnect();
        mqttConnected = false;
    } else if (mqttEnabled) {
        setupMQTT();
    ...
    server.send(200, "application/json", 
        "{\"success\":true,\"enabled\":" + String(mqttEnabled ? "true" : "false") + "}");
}
void handleReset() {
    if (!checkApiRateLimit()) {
        server.send(429, "application/json", "{\"success\":false}");
