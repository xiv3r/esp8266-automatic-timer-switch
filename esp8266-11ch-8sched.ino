...
// Removed unused lastRTCUpdate and RTC_UPDATE_INTERVAL
...
<html>
<head>
    <title>ESP8266 11-Channel Relay Smart Switch</title>
...
<body>
    <div class="container">
        <div class="header">
            <h1>ESP8266 11-Channel Relay Smart Switch</h1>
    ...
    <script>
        ...
        function pollScanResults() {
            ...
            fetch('/api/wifi/scan')
                ...
                .then(data => {
                    if (data.status === 'scanning') {
                        // Still scanning, poll again after 1 second
        ...
        // Load current WiFi settings on page load
        loadWiFi();
    </script>
</body>
</html>
    ...
    <script>
        ...
        function loadAP() {
            fetch('/api/ap')
                .then(response => response.json())
                .then(data => {
                    document.getElementById('ap_ssid').value = data.ap_ssid || 'ESP8266_11CH_Timer_Switch';
...
// CHANGED: Now applies drift compensation and handles millis() rollover
time_t getCurrentEpoch() {
    // If WiFi is connected and NTP time is valid, use it directly
    if (wifiConnected) {
    ...
    // Fallback to internal RTC with drift compensation
    if (rtcInitialized) {
        unsigned long now = millis();
        unsigned long elapsed;
        // Handle millis() rollover (every ~50 days)
        ...
        // Apply drift compensation factor
...
// CHANGED: Improved drift calculation with exponential moving average and rollover safety
void syncInternalRTC() {
    if (wifiConnected) {
        ...
        if (ntpEpoch > 100000) {
            ...
            if (rtcInitialized) {
                ...
                // Only update drift after at least 10 seconds to get a meaningful measurement
                if (elapsed > 10000) {
                    float actualDrift = (float)(ntpEpoch - internalEpoch) / (elapsed / 1000.0);
                    // Exponential moving average (alpha = 0.05) for smooth adaptation
...
void loop() {
    ...
    // Update NTP time (now only every hour instead of every second)
...
void setupWebServer() {
    ...
    server.on("/api/wifi/scan", HTTP_GET, handleWiFiScan);   // NEW: WiFi scan endpoint
...
void handleWiFiScan() {
    // NEW: WiFi scan handler
    int n = WiFi.scanComplete();
    
    // If scan hasn't been started, start it now (async)
    ...
    // Scan finished, build JSON response
    DynamicJsonDocument doc(4096);
    ...
    for (int i = 0; i < n; ++i) {
        ...
        net["encryption"] = (WiFi.encryptionType(i) == ENC_TYPE_NONE) ? "Open" : "Secured";
        // Add a combined display string for convenience
        char display[64];
