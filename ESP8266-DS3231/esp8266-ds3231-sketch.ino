/*
============================================================
 *  ESP8266 6-Channel Relay Smart Switch w/ DS3231 RTC & LED Status Indicator
 *  Author: Raff Alds
...
static const unsigned long RTC_SYNC_INTERVAL = 60000UL;
...
static const unsigned long RECOVERY_DELAY       = 2000UL;
...
enum RecoveryAction {
    RECOVERY_NONE,
    RECOVERY_WIFI_RESET,
    RECOVERY_WEB_SERVER_RESET,
    RECOVERY_FULL_SOFT_RESET,
    RECOVERY_RTC_RESET,
    RECOVERY_TIMER_RESET,
    RECOVERY_MEMORY_CLEANUP
};

RecoveryAction pendingRecovery = RECOVERY_NONE;
unsigned long recoveryTimestamp = 0;
uint8_t recoveryAttempts = 0;
static const uint8_t MAX_RECOVERY_ATTEMPTS = 5;
unsigned long lastAutoHealCheck = 0;
...
// ─── Recovery System──────────────────────────────────────────────
void softRecovery(RecoveryAction action);
...
.ibar{display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:8px;margin-bottom:16px}
...
<input type="text" id="srv" placeholder="nts.netnod.se" required>
...
fetch('/api/ntp').then(r=>r.json()).then(d=>{
  document.getElementById('srv').value=d.ntpServer||'nts.netnod.se';
...
<div class="card fcrd">
<div class="alert ai">&#x26A0;&#xFE0F; Pin changes require device restart. D1 and D2 are reserved for DS3231 RTC.</div>
...
<p style="font-size:12px;color:#90A4AE;margin-bottom:10px">Click an available pin to add a relay on that pin (D1/D2 are reserved for DS3231 RTC)</p>
...
function resetPinsToDefault(){
  if(!confirm('Reset ALL GPIO pins to default (D0,D5,D6,D7,RX,TX)?\n\nRelay names and schedules will be preserved, but pin assignments will be reset to factory defaults.\nDevice will restart to apply changes.')) return;
  ...
  .then(d => {
    if(d.success) {
      toast('GPIO pins reset! Restarting...');
...
<div class="ibox"><div class="l">DS3231 RTC</div><div class="v" id="sds">&hellip;</div></div>
</div>

<div class="card fcrd" style="margin-bottom:14px">
<p style="font-weight:700;margin-bottom:12px">mDNS</p>
<div class="fg">
<small>Accessible as http://esp8266relay.local</small>
...
function loadSys(){
  fetch('/api/system').then(r=>r.json()).then(d=>{
    ...
    document.getElementById('sds').textContent=d.ds3231?'✓ Present (D1/D2)':'✗ Not found';
  }).catch(()=>{});
}
...
void checkFactoryResetButton() {
    ...
    static bool visualFeedbackActive = false;
    static unsigned long lastVisualBlink = 0;    
    ...
    if ((millis() - lastDebounce) > 50) {
        if (currentState == LOW && !isPressing && !factoryResetInProgress) {
            isPressing = true;
            factoryResetPressStart = millis();
            visualFeedbackActive = true;
            lastVisualBlink = millis();
        }
        else if (currentState == HIGH && isPressing) {
            unsigned long holdDuration = millis() - factoryResetPressStart;
            if (holdDuration >= FACTORY_RESET_HOLD_MS && !factoryResetInProgress) {
                performFactoryReset();
            }
            isPressing = false;
    ...
    if (visualFeedbackActive && !factoryResetInProgress) {
        unsigned long holdTime = millis() - factoryResetPressStart;        
        if (holdTime >= FACTORY_RESET_HOLD_MS) {
            ...
            if (millis() - lastVisualBlink >= (unsigned long)blinkInterval) {
                lastVisualBlink = millis();
                ledState = !ledState;
...
void softRecovery(RecoveryAction action) {
    if (pendingRecovery != RECOVERY_NONE) return;
    
    if (action == RECOVERY_FULL_SOFT_RESET && recoveryAttempts < 3) {
       
        if (!wifiConnected) action = RECOVERY_WIFI_RESET;
        else if (ESP.getFreeHeap() < 8192) action = RECOVERY_MEMORY_CLEANUP;
        else action = RECOVERY_WEB_SERVER_RESET;
    }
    
    if (recoveryAttempts >= MAX_RECOVERY_ATTEMPTS) {       
        recoveryAttempts = 0;
        pendingRecovery = RECOVERY_NONE;      
        action = RECOVERY_WIFI_RESET;
    }
    
    preserveRelayState();
    pendingRecovery = action;
    recoveryTimestamp = millis();
    recoveryAttempts++;    
    flushConfigIfNeeded();
    saveRTCState();
}
...
void executeRecovery() {
    ...
    switch (pendingRecovery) {
        ...
        case RECOVERY_WEB_SERVER_RESET:         
            server.stop();
            server.close();
            delay(100);
            setupWebServer();
    ...
    unsigned long now = millis();
    if (now - lastStableTime > 3600000UL) {
        if (recoveryAttempts > 0) recoveryAttempts--;
        lastStableTime = now;
    }
}
...
void autoHealCheck() {
    unsigned long now = millis();
    if (now - lastAutoHealCheck < AUTO_HEAL_INTERVAL) return;
    ...
    if (wifiConnected && WiFi.status() != WL_CONNECTED) {
        ...
        softRecovery(RECOVERY_WIFI_RESET);
        return;
    }
    ...
    if (ESP.getFreeHeap() < 8192) {
        performMemoryCleanup();       
        if (ESP.getFreeHeap() < 4096) {
            softRecovery(RECOVERY_MEMORY_CLEANUP);
        }
        return;
    }
    
    time_t currentEpoch = getCurrentEpoch();
    if (rtcInitialized && (currentEpoch < 1000000000UL || currentEpoch > 5000000000UL)) {     
        if (lastNTPSyncEpoch > 0 && abs((long)currentEpoch - (long)lastNTPSyncEpoch) > 86400) {
            softRecovery(RECOVERY_RTC_RESET);
        }
        return;
    }
    
    static unsigned long lastServerCheck = 0;
    if (now - lastServerCheck > 300000UL) {       
        lastServerCheck = now;
    }
}
...
void gentleNetworkHealing() {
    ...
    if (!wifiConnected && !networkHealingInProgress && strlen(sysConfig.sta_ssid) > 0) {
        ...
        while (millis() - start < 30000 && networkHealingInProgress) {
            if (WiFi.status() == WL_CONNECTED) {
        ...
        networkHealingInProgress = false;
    } else if (networkHealingInProgress && wifiConnected) {
        networkHealingInProgress = false;
    }
}
...
void syncFromDS3231() {
    ...
    if (rtcEpoch > 1000000000UL && rtcEpoch < 5000000000UL) {
        unsigned long nowMs = millis();
...
void updateStatusLED() {
    if (factoryResetButtonPressed || factoryResetInProgress) return;
    
    unsigned long now = millis();
    unsigned long interval;    
    if (pendingRecovery != RECOVERY_NONE) {
        interval = 100UL;
...
void initDefaults() {
    ...
    strcpy(sysConfig.ntp_server, "nts.netnod.se");
...
void loop() {
    ...
    if (pendingRecovery != RECOVERY_NONE) {
        if (now - recoveryTimestamp >= RECOVERY_DELAY) {
            executeRecovery();
            return;
        }
        dnsServer.processNextRequest();
    ...
    if (rtcPresent && !wifiConnected && rtcInitialized && 
        elapsedSince(lastRTCSync) >= RTC_SYNC_INTERVAL) {
        lastRTCSync = millis();
        syncFromDS3231();
    }
    ...
    if (elapsedSince(lastSuccessfulLoop) > LOOP_WATCHDOG_TIMEOUT) {
        softRecovery(RECOVERY_FULL_SOFT_RESET);
        lastSuccessfulLoop = now;
        return;
    }
...
void handleGetSystem() {
    ...
    resp += "\"ds3231\":" + String(rtcPresent ? "true" : "false");
    resp += "}";
    server.send(200, "application/json", resp);
}
...
void handleSavePins() {
    ...
    savePinConfig();
    saveConfiguration();    
    server.send(200, "application/json", "{\"success\":true,\"softReset\":true}");
    softRecovery(RECOVERY_TIMER_RESET);
}
...
void handleReset() {
    flushConfigIfNeeded();
    server.send(200, "application/json", "{\"success\":true,\"recovery\":\"wifi_reset\"}");    
    softRecovery(RECOVERY_WIFI_RESET);
}
