/*
============================================================
 *  ESP8266 9-Channel Relay Smart Switch w/ LED Status Indicator
 *  Author: Raff Alds
 *  Github: https://www.github.com/xiv3r
 *  Project: Home, Business, Farm Automation etc...
 *  Modified: Added DS3231 RTC support, removed D1/D2 from relay pins
============================================================
 */
...
static const unsigned long CONFIG_SAVE_INTERVAL =   30000UL; 
...
char ap_ssid[32]     = "ESP8266_9CH_Timer_Switch";
...
inline const char* getPinName(uint8_t pin) {
    switch(pin) {
        case 16: return "D0";
        case 5:  return "D1 (I2C SCL)";
        case 4:  return "D2 (I2C SDA)";
        case 0:  return "D3";
        case 2:  return "D4";
        case 14: return "D5";
        case 12: return "D6";
...
bool loadFromFile(const void* data, size_t size);
...
// ─────────────────────────────────────────────────────────────────────────────
//  HTML (all pages remain unchanged from original)
// ─────────────────────────────────────────────────────────────────────────────
...
<header>
<span class="logo">&#x26A1; ESP8266 9-CH</span>
<nav>
...
// [All remaining HTML pages (wifi_html, ntp_html, ap_html, pins_html, system_html) remain identical to original]
// I'll include them here for completeness but they're unchanged
...
function save(){
  ...
  .then(r=>r.json()).then(d=>{if(d.success){toast('Saved! Connecting to new network\u2026');}else toast('Failed: '+d.error,false);})
  .catch(()=>toast('Error',false));
}
</script></body></html>)raw";
...
<button class="btn bbrowser" id="bbtn" onclick="syncFromBrowser()" style="padding:9px 16px;border-radius:6px;font-size:13px;font-weight:600;margin-top:8px;white-space:nowrap">&#x1F310; Sync from Browser</button>
</div>
</div>
</main>
<div id="toast"></div>
...
<html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Pins — ESP8266 Timer Switch</title>
<link rel="stylesheet" href="/style.css"></head><body>
<header>
<span class="logo">&#x26A1; ESP8266 9-CH</span>
<nav>
...
<div class="card fcrd">
<div class="alert ai">&#x26A0;&#xFE0F; Pin changes require device restart. Reserved pins: D1(GPIO5 I2C SCL), D2(GPIO4 I2C SDA), D4(GPIO2 STATUS LED), D8(GPIO15)</div>

<div class="fg">
<label>Global Relay Logic</label>
<select id="globalActiveLow">
<option value="1">Active LOW (relay ON when GPIO LOW - most common)</option>
<option value="0">Active HIGH (relay ON when GPIO HIGH)</option>
</select>
</div>
...
<button class="btn bsave" onclick="savePins()" style="padding:9px 16px">&#x1F4BE; Save &amp; Restart</button>
</div>
</div>
...
<div id="toast"></div>
<script>
let pinCfg={numRelays:9,globalActiveLow:true,relays:[]};
const pinNames={16:'D0',0:'D3',14:'D5',12:'D6',13:'D7',3:'RX',1:'TX'};
const validPins=[16,0,14,12,13,3,1];
...
function load(){
  fetch('/api/pins').then(r=>r.json()).then(d=>{
    pinCfg=d;
    document.getElementById('globalActiveLow').value=d.globalActiveLow?'1':'0';
    render();
...
function render(){
  ...
  pinCfg.relays.forEach((r,i)=>{
    ...
    tr.innerHTML='<td><strong>Relay '+(i+1)+'</strong></td>'+
      ...
      '<td><select onchange="pinCfg.relays['+i+'].activeLow=this.value===\'1\'" style="padding:5px;border:1px solid #CFD8DC;border-radius:4px"><option value="1" '+(r.activeLow?'selected':'')+'>Active LOW</option><option value="0" '+(!r.activeLow?'selected':'')+'>Active HIGH</option></select></td>'+
...
function renderAvail(){
  ...
  c.innerHTML='';
  validPins.forEach(p=>{
    const used2=used.includes(p);
    const d=document.createElement('span');
    d.className='pin-chip '+(used2?'used':'available');
    d.textContent=pn(p);
    if(!used2){
...
function addRelayOnPin(pin){
  ...
  pinCfg.relays.push({name:'Relay '+(pinCfg.relays.length+1),pin:pin,activeLow:pinCfg.globalActiveLow===true});
  pinCfg.numRelays=pinCfg.relays.length;
  render();
  toast('Added on '+pn(pin));
}

function addRelay(){
  const used=pinCfg.relays.map(r=>r.pin);
  const avail=validPins.find(p=>!used.includes(p));
  if(!avail){toast('No pins available',false);return;}
  addRelayOnPin(avail);
}
...
function savePins(){
  if(!confirm('Save pins and restart device?'))return;
  ...
  .then(r=>r.json()).then(d=>{
    if(d.success){toast('Saved! Restarting...');setTimeout(()=>location.reload(),4000);}
    else toast('Failed: '+(d.error||''),false);
  }).catch(()=>toast('Error',false));
}
load();
...
bool initDS3231() {
    ...
    if (rtc.begin()) {
        ...
        Serial.println("DS3231 RTC detected and initialized");
        
        if (rtc.lostPower()) {
            Serial.println("DS3231 lost power, setting time from internal RTC if available");
            if (internalEpoch > 1000000000UL) {
                rtc.adjust(DateTime(internalEpoch));
            }
        }
        ...
        Serial.println("DS3231 NOT detected - using NTP/internal RTC only");
        return false;
    }
}
...
time_t getCurrentEpoch() {
    ...
    // Priority 2: Internal software RTC (synced from NTP or DS3231)
    if (!rtcInitialized || internalEpoch == 0) return 0;
    
    unsigned long now = millis();
    unsigned long elapsed;
    
    if (now >= internalMillisAtLastSync) {
        elapsed = now - internalMillisAtLastSync;
    } else {
        elapsed = (0xFFFFFFFFUL - internalMillisAtLastSync) + now + 1;
    }
...
void syncInternalRTC() {
    ...
    if (rtcInitialized && internalEpoch > 0) {
        ...
        if (nowMs >= internalMillisAtLastSync) {
            elapsedMs = nowMs - internalMillisAtLastSync;
        } else {
            elapsedMs = (0xFFFFFFFFUL - internalMillisAtLastSync) + nowMs + 1;
        }
...
void syncInternalRTCFromDS3231() {
    ...
    if (rtcInitialized && internalEpoch > 0) {
        ...
        if (elapsedMs > 60000UL) {
            float nominalSecs = (float)elapsedMs / 1000.0f;
            float actualSecs = (float)((long)dsEpoch - (long)internalEpoch);
            float measuredRate = actualSecs / nominalSecs;
            driftCompensation = driftCompensation * 0.75f + measuredRate * 0.25f;
            if (driftCompensation < 0.95f) driftCompensation = 0.95f;
            if (driftCompensation > 1.05f) driftCompensation = 1.05f;
...
void syncInternalRTCFromBrowser(time_t browserEpoch) {
    ...
    // Update DS3231 if available
    if (ds3231Available) {
        rtc.adjust(DateTime(browserEpoch));
    }
    ...
    if (rtcInitialized && internalEpoch > 0) {
        ...
        if (elapsedMs > 60000UL) {
            float nominalSecs = (float)elapsedMs / 1000.0f;
            float actualSecs = (float)((long)browserEpoch - (long)internalEpoch);
            float measuredRate = actualSecs / nominalSecs;
            driftCompensation = driftCompensation * 0.75f + measuredRate * 0.25f;
            if (driftCompensation < 0.95f) driftCompensation = 0.95f;
            if (driftCompensation > 1.05f) driftCompensation = 1.05f;
...
void tryNTPSync() {
    ...
    // If DS3231 is available, only sync NTP periodically to correct drift
    if (ds3231Available && lastNTPSync > 0 && millis() - lastNTPSync < 43200000UL) { // 12 hours
        return;
    }
...
void beginWiFiConnect() {
    ...
    if (reconnectCountThisHour >= MAX_RECONNECT_ATTEMPTS) {
        wifiGiveUpUntil = millis() + RECONNECT_COOLDOWN;
        reconnectCountThisHour = 0;
        return;
    }
...
void updateStatusLED() {
    ...
    } else if (!rtcInitialized || (lastNTPSync == 0 && !ds3231Available)) {
...
void setup() {
    ...
    // Initialize I2C for DS3231 on D1(SCL) and D2(SDA)
    Wire.begin(I2C_SDA_PIN, I2C_SCL_PIN);
    ...
    // Initialize DS3231 as primary time source
    initDS3231();
    
    // If DS3231 is available, sync internal RTC from it
    if (ds3231Available) {
...
void loop() {
    ...
    // Periodic DS3231 sync (every 10 seconds)
    if (ds3231Available && now - lastRTCUpdate >= 10000UL) {
        lastRTCUpdate = now;
        syncInternalRTCFromDS3231();
    }
    ...
    if (wifiConnected) {
        bool doSync = false;
        if (lastNTPSync == 0 && !ds3231Available) {
            doSync = true;
...
// =============================================================================
//  CONFIG (unchanged defaults except pin assignments)
// =============================================================================
...
void initDefaults() {
    ...
    strcpy(sysConfig.ap_ssid, "ESP8266_9CH_Timer_Switch");
    ...
    pinConfig.numRelays = 7;  // Reduced from 9 to 7 (removed D1 and D2)
    pinConfig.globalActiveLow = true;
    numRelays = 7;
    relayActiveLow = true;
    
    // Updated pin assignments - D1(5) and D2(4) removed, reserved for I2C
    uint8_t defaultPins[] = {16, 0, 14, 12, 13, 3, 1};  // D0, D3, D5, D6, D7, RX, TX
...
void loadConfiguration() {
    ...
    if (!loadFromFile(RELAY_FILE, relayConfigs, sizeof(RelayConfig) * MAX_RELAYS)) {
        ...
        for (int i = 0; i < numRelays; i++) {
            ...
            for (int s = 0; s < 8; s++) {
                if (relayConfigs[i].schedule.days[s] == 0) {
                    relayConfigs[i].schedule.days[s] = 0x7F;
                }
            }
        }
...
void loadPinConfig() {
    if (!loadFromFile(PINS_FILE, &pinConfig, sizeof(PinConfig))) {
        pinConfig.magic = PINS_MAGIC;
        pinConfig.numRelays = 7;
        pinConfig.globalActiveLow = true;
        numRelays = 7;
        relayActiveLow = true;
        savePinConfig();
        
        uint8_t defaultPins[] = {16, 0, 14, 12, 13, 3, 1};
...
void handleNtpPage() { server.send_P(200, "text/html", ntp_html); }
void handleApPage() { server.send_P(200, "text/html", ap_html); }
void handleSystemPage() { server.send_P(200, "text/html", system_html); }
void handlePinsPage() { server.send_P(200, "text/html", pins_html); }
void handleStyleCss() { server.send_P(200, "text/css", style_css); }
void handleCaptivePortal() { server.send(200, "text/html", "<HTML><HEAD><TITLE>Success</TITLE></HEAD><BODY>Success</BODY></HTML>"); }
void handleSuccess() { server.send(200, "text/plain", "success\n"); }
void handleConnectTest() { server.send(200, "text/plain", "Microsoft Connect Test"); }
void handleNcsi() { server.send(200, "text/plain", "Microsoft NCSI"); }
...
// =============================================================================
//  API (unchanged except handleGetSystem for DS3231 status)
// =============================================================================
...
void handleGetTime() {
    ...
    server.send(200, "application/json", "{\"time\":\"" + ts + "\",\"wifi\":" + String(wifiConnected ? "true" : "false") + ",\"ntp\":" + String((lastNTPSync > 0 || ds3231Available) ? "true" : "false") + ",\"rtc\":" + String(ds3231Available ? "true" : "false") + "}");
}
...
void handleBrowserTimeSync() {
    ...
    if (browserEpoch < 1000000000UL || browserEpoch > 2000000000UL) {
...
void handleGetWiFi() {
    server.send(200, "application/json", "{\"ssid\":\"" + String(sysConfig.sta_ssid) + "\",\"connected\":" + String(wifiConnected ? "true" : "false") + ",\"ip\":\"" + WiFi.localIP().toString() + "\",\"rssi\":" + String(wifiConnected ? (int)WiFi.RSSI() : 0) + "}");
}
...
void handleGetSystem() {
    ...
    resp += "\"ntpSynced\":" + String((lastNTPSync > 0 || ds3231Available) ? "true" : "false") + ",";
    resp += "\"ds3231\":" + String(ds3231Available ? "true" : "false") + ",";
