...
function renderNets(nets){
  const nl=document.getElementById('netlist');
  if(!nets.length){
    nl.innerHTML='<div style="color:#90A4AE;text-align:center;padding:8px;font-size:13px">No networks found.</div>';
    return;
  }
  const frag=document.createDocumentFragment();
  const hdr=document.createElement('div');
  hdr.className='net-hdr';
  hdr.textContent='Available Networks (click to select)';
  frag.appendChild(hdr);
  
  nets.sort((a,b)=>b.rssi-a.rssi).forEach(n=>{
    const d=document.createElement('div');
    d.className='netitem';
    d.style.cursor='pointer';  // Add explicit cursor pointer
    
    const ns=document.createElement('span');
    ns.className='ns';
    ns.textContent=n.ssid;
    
    const nr=document.createElement('span');
    nr.className='nr';
    nr.textContent=n.rssi+'dBm';
    
    const bar=document.createElement('span');
    bar.innerHTML=rssiBar(n.rssi);
    
    const lock=document.createElement('span');
    lock.style.fontSize='13px';
    lock.textContent=n.enc?'\uD83D\uDD12':'\uD83D\uDD13';
    ...
    d.addEventListener('click', function(e) {
      ...
      // Set the value and trigger proper events
      ...
      // Focus the password field if the network is encrypted
      if(n.enc) {
        pwInput.focus();
        toast('Selected: ' + escapeHtml(n.ssid) + ' (password required)');
      } else {
        toast('Selected: ' + escapeHtml(n.ssid) + ' (open network)');
      }
    });
    ...
    d.setAttribute('aria-label', 'Select network ' + n.ssid);
...
<div class="card fcrd">
<div class="alert aw">&#x26A0;&#xFE0F; Changes will apply immediately.</div>
...
<div class="card fcrd">
<div class="alert ai">&#x26A0;&#xFE0F; Changes apply immediately.</div>
