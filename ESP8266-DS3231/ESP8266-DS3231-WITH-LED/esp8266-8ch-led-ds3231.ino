 ...
 *  Feature: HH:MM:SS + Day-of-Week + Day-of-Month Schedules
 *  RTC: DS3231 on I2C (D1=SCL, D2=SDA)
============================================================
 */
...
#define STATUS_LED_PIN D4  // GPIO2 - Built-in LED on ESP8266 (active LOW)
static const unsigned long LED_FAST_BLINK   = 200UL;  // Fast blink: WiFi disconnected
static const unsigned long LED_SLOW_BLINK   = 1000UL; // Slow blink: WiFi connected, NTP not synced
...
// ─── Relay config ────────────────────────────────────────────────────────────
#define NUM_RELAYS 8  // Reduced to 8 (D4 removed)
const int  relayPins[NUM_RELAYS] = { D0, D3, D5, D6, D7, D8, 3, 1 };
...
bool          ledState      = false;
unsigned long currentLEDInterval = LED_FAST_BLINK;
...
// =============================================================================
//  ATOMIC FILE I/O WITH POWER-LOSS PROTECTION
// =============================================================================
...
void updateStatusLED() {
    ...
    bool ntpSynced = (lastNTPSync > 0);
    
    // Determine LED blink interval based on status
    if (!wifiConnected) {
        // WiFi disconnected: Fast blink
        currentLEDInterval = LED_FAST_BLINK;
    } else if (!ntpSynced) {
        // WiFi connected, NTP not synced: Slow blink
        currentLEDInterval = LED_SLOW_BLINK;
    } else {
        // WiFi connected and NTP synced: LED OFF
        digitalWrite(STATUS_LED_PIN, HIGH); // Active LOW, so HIGH = OFF
        lastLEDToggle = now;
        return;
    }
    
    // Handle blinking
    if (now - lastLEDToggle >= currentLEDInterval) {
        ledState = !ledState;
        digitalWrite(STATUS_LED_PIN, ledState ? LOW : HIGH); // Active LOW
        lastLEDToggle = now;
    }
}
...
void setup() {
    ...
    digitalWrite(STATUS_LED_PIN, HIGH); // Active LOW, so HIGH = OFF initially

    // Initialize relay pins (8 channels)
    ...
    // Connect to WiFi Station if configured
